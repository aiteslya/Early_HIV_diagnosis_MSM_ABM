function []=Main_Traj_Load_ART_Sens(par_counter,batch_n, sim_type)
% this function runs forward scenarios where for the accelerated diagnosis
% in AEHI there is no immediate ART initiation
% input parameters
        ...
        if burn_fl % burn in is over, run the HIV dynamics
            ...
            if numel(ind_AIDS)>0
                ...
                if num_died_HIV>0
                    ...
                    total_treat_ind=setdiff(total_treat_ind,ind_dead);% container for these who are on treatment but not suppressed yet
...
% %     
  ...
  %       toc
            ...
            if numel(infect_undiagn_old)>0 % if number of infected and undiagnosed prior to this stage is above zero
                [Pop_hiv,newly_diagn,prop_diagn_AHI(traj_counter,1),new_total_diagn_ind,new_AHI_diagn_ind]=diagnose_Load_No_ART(Pop_hiv,hiv_index,HIV_st,cfg,infect_undiagn_old,prop_diagn_AHI(traj_counter,1),tcounter,cfg_screen,new_total_diagn_ind,new_AHI_diagn_ind);
...
%             tic 
            ...
            % code for the immediate initiation of ART when diagnosed as
            % AEHI
            ...
            % else
             %    total_diagn_ind = [total_diagn_ind, newly_diagn];
            % end  

            % code where people who are diagnosed as a part of AEHI go to
            % the treatment at the same rate
            total_diagn_ind = [total_diagn_ind, newly_diagn];

            % add immigrated diagnosed
