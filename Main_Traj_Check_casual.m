function []=Main_Traj_Check_casual(par_counter,batch_n)
...
% file names
% age distribution for the population at the 
% start of the run, between the ages of 15 and 75 by increments of 10, 
% should add up to 1
AgeFileStr='Age_distr_2014.csv'; 
...
% OutFolderStr=[out_folder_str,'_par_num_',num2str(par_counter),'_batch_',num2str(batch_n)];
...
AgeDistr=readmatrix(fullfile(DataFolderStr,AgeFileStr)); % 6 by 1 matrix
...
T_burn_d = year*cfg.T_burn; % convert burn-in time of demographics and sexual networks from days to years
...
inf_source_casual_prop_distr=zeros(cfg.n_traj, max_num_casual + 50); % 50 is padding

% the infectee
inf_target_age_distr=zeros(cfg.n_traj,num_age);
inf_target_HIVSt_distr=zeros(cfg.n_traj,25);
inf_target_casual_prop_distr=zeros(cfg.n_traj, max_num_casual + 50);
...
for traj_counter=1:1:cfg.n_traj
    ...
    yHIV=zeros(ceil((cfg.T*year-cfg.T_burn*year)/write_step),numel(fieldnames(HIV_st)));
    ...
    % select age bins
    % Pop_age_bins=randsample(age_bins, cfg.N, true, AgeDistr);
    Pop_age_bins=randsample(1:6, cfg.N, true, N_prop);
    %Pop_age_bins = discretize(Pop_age,15:10:75);

    % set ages
    Population.Data(pop_index.age,1:cfg.N)=unidrnd(band_width,1,cfg.N)+min_age+(Pop_age_bins-1)*cfg.AgeBr-1;
    % assign age bands according to the age
    Population.Data(pop_index.age_bin,1:cfg.N) = Pop_age_bins; 
    ...
    % initialize the desire to form casual relationships
    ...
    % create new steady partnerships until reached the value of the steady
    % state distribution
    n_single_initial = n_single;

    while n_single > steady_equil_distr(1)*cfg.N
        s_fl=0;
        while ~s_fl
            [Population,s_fl,Rels_steady] = create_steady_pair(Population,pop_index,tcounter,Rels_steady,alive_ind,age_attr_steady,sero_attr_steady,Pop_hiv,hiv_index,all_casual_parts);
        end
        % re-calculate the number of single individuals
        n_single=n_single-2;
    end
    ...
    j_values = randsample(1:numel(steady_dur_distr),num_pairs,true,steady_dur_distr);
...
%       tic
        ...
        %[Population,Pop_hiv,Id_counter,alive_ind,ind_new_bracket]=Birth(Population,Pop_hiv,fl_collect,pop_index,hiv_index,lambda_d,tcounter,Id_counter,alive_ind);
        if tcounter>=2.5*year
            current_year = 2014 + floor((tcounter-182)/365);
        else
            current_year = 2014 + floor(tcounter/365);
        end

       % current_year
        lambda_diagn_d = diagn_immigr_FV(current_year-2013, 2)/year;

        [Population,Pop_hiv,Id_counter,alive_ind,ind_new_bracket, newly_infect3, new_diagn, new_suppr, infect_alive]=Birth_Immigr(Population,Pop_hiv, pop_index,hiv_index,lambda_d,tcounter,Id_counter,alive_ind, cfg, lambda_diagn_d, infect_alive, HIV_st);
...
%           % PrEP processes
%             
             if tcounter>=(cfg_infect.prep_prop_init_time)*year
                % disp('PrEP uptake');
                % tic 
    ...
    % %     
      ...
      %       toc
            ...
            if numel(infect_undiagn_old)>0 % if number of infected and undiagnosed prior to this stage is above zero
                [Pop_hiv,newly_diagn,prop_diagn_AHI(traj_counter,1),new_total_diagn_ind,new_AHI_diagn_ind]=diagnose(Pop_hiv,hiv_index,HIV_st,cfg,infect_undiagn_old,prop_diagn_AHI(traj_counter,1),tcounter,cfg_screen,new_total_diagn_ind,new_AHI_diagn_ind);
                if numel(newly_diagn)>0
                    ...
                    for ind_counter=1:numel(newly_diagn)
                        ...
                        if time_infect==cfg.T_burn*year+1 % initial infected
                            if HIV_stat==HIV_st.A1 % was initially infected when diagnosed, collect their
                                infect_age = tcounter-cfg.T_burn*year;
...
%       points
    ...
    %S_TS(traj_counter,:)=interp1(0:1:(cfg.T*year),yHIV(:,HIV_st.susc+1),tdistr);
    % new  code - first parameter of interp1
    ...
    if ~isempty(num_new_infect)   
        ...
        if tcounter>(cfg_screen.T_extra_s*year+10)
